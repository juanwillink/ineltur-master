    ...
    public partial class Manager
    {
        protected class InternationalManager : ISearchManager
        {
            ...
            public LodgingListModel SearchLodging(SearchLodgingRequestModel searchLodgingRequestModel, Credential userCredential)
            {
                ...
                results.Lodgings = CompleteLodgingInformation(lodgings);

                return results;
            }

            private List<LodgingModel> CompleteLodgingInformation(List<LodgingModel> lodgings)
            {
                var rqdetails = new WSHCEV_main.HotelDetailsSearchRQ();
                var x = new System.Xml.Serialization.XmlSerializer(rqdetails.GetType());
                var sw = new System.IO.StringWriter();
                var xw = new System.Xml.XmlTextWriter(sw);
                var wsRnd = new ServicioPruebaHeredado();
                ...
                try
                {
                    rqdetails.Source = new WSHCEV_main.Source();
                    rqdetails.Source.PricingSurferConfiguration = new WSHCEV_main.PricingSurferConfiguration();
                    rqdetails.Source.PricingSurferConfiguration.PreferedLanguage = new WSHCEV_main.PreferedLanguage();
                    rqdetails.Source.PricingSurferConfiguration.PreferedCurrency = new WSHCEV_main.PreferedCurrency();
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet = new WSHCEV_main.ResponseSet();
                    rqdetails.Source.PricingSurferConfiguration.ContractId = 10273; // 10857;
                    rqdetails.Source.PricingSurferConfiguration.PreferedLanguage.LanguageCode = "es";
                    rqdetails.Source.PricingSurferConfiguration.PreferedCurrency.CurrencyCode = "USD";
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet.FirstItem = 1;
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet.ItemsPerPage = 1500;

                    rqdetails.Details = new WSHCEV_main.HotelDetailsSearchRQDetails();
                    rqdetails.Details.Criterion = new WSHCEV_main.HotelDetailsSearchRQDetailsCriterion();
                    rqdetails.Details.Criterion.HotelCodes = new WSHCEV_main.HotelCodes();

                    rqdetails.Details.Criterion.HotelCodes.HotelCode = lodgings.Select(l => new WSHCEV_main.HotelCode{SupplierID = l.LodgingSupplierId, Value = l.LodgingId}).ToArray();
                    rqdetails.Details.Criterion.HotelCodes.ItemsCount = rqdetails.Details.Criterion.HotelCodes.HotelCode.Length;
                    rqdetails.Details.Criterion.HotelCodes.OrderBy = "ASC";

                    xw.Formatting = System.Xml.Formatting.None;
                    x.Serialize(xw, rqdetails);

                    wsRnd.Timeout = 30000;
                    var response = wsRnd.HotelDescriptiveInfo(sw.ToString());
                    var sr = new StringReader(response);

                    x = new System.Xml.Serialization.XmlSerializer(typeof(WSHCEV_main.HotelDetailsSearchRS));
                    var lodgingListRS = (WSHCEV_main.HotelDetailsSearchRS)x.Deserialize(sr);
                    sr.Close();

                    if (lodgingListRS != null && lodgingListRS.Item is WSHCEV_main.HotelDetailsSearchRSDetails &&
                        ((WSHCEV_main.HotelDetailsSearchRSDetails)lodgingListRS.Item).Hotels != null)
                    {
                        foreach (var detail in ((WSHCEV_main.HotelDetailsSearchRSDetails)lodgingListRS.Item).Hotels.Hotel)
                        {
                            var lodging = lodgings.SingleOrDefault(l => l.LodgingId == detail.HotelCode);

                            if (detail.HotelDescriptions != null && detail.HotelDescriptions.ItemsCount > 0)
                            {
                                lodging.LodgingDescription = detail.HotelDescriptions.HotelDescription[0].Value;
                            }

                            if (detail.ImageLinks != null && detail.ImageLinks.ItemsCount > 0)
                            {
                                lodging.LodgingPhoto = detail.ImageLinks.ImageLink[0].ImageURL;
                            }

                            CompleteCancelationCharges(lodging);
                        }
                    }
                }
                catch (Exception ex)
            ...
            private LodgingModel CompleteCancelationCharges(LodgingModel lodging)
            {
                var rqcancelation = new WSHCEV_main.BookingServiceChargeConditionsRQ();
                var x = new System.Xml.Serialization.XmlSerializer(rqcancelation.GetType());
                var sw = new System.IO.StringWriter();
                var xw = new System.Xml.XmlTextWriter(sw);
                var wsRnd = new ServicioPruebaHeredado();
                ...
                try
                {
                    rqcancelation.Source = new WSHCEV_main.Source();
                    rqcancelation.Source.PricingSurferConfiguration = new WSHCEV_main.PricingSurferConfiguration();
                    rqcancelation.Source.PricingSurferConfiguration.PreferedLanguage = new WSHCEV_main.PreferedLanguage();
                    rqcancelation.Source.PricingSurferConfiguration.PreferedCurrency = new WSHCEV_main.PreferedCurrency();
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet = new WSHCEV_main.ResponseSet();
                    rqcancelation.Source.PricingSurferConfiguration.ContractId = 10273; // 10857;
                    rqcancelation.Source.PricingSurferConfiguration.PreferedLanguage.LanguageCode = "es";
                    rqcancelation.Source.PricingSurferConfiguration.PreferedCurrency.CurrencyCode = "USD";
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet.FirstItem = 1;
                    //rqdetails.Source.PricingSurferConfiguration.ResponseSet.ItemsPerPage = 1500;

                    rqcancelation.Details = new WSHCEV_main.BookingService[1];
                    ...
                    {
                        ItemsCount = 1,
                        BookingServiceItem = lodging.Vacancies.Select(v => new WSHCEV_main.BookingServiceItem
                        {
                            Sequence = 1,
                            SupplierID = lodging.LodgingSupplierId,
                            DestinationDetails = new WSHCEV_main.DestinationDetails
                            {
                                ...
                                Destination = new WSHCEV_main.Destination[]
                                {
                                    ...
                                    {
                                        Sequence = 1,
                                        DestinationCode = lodging.DestinationId,
                                        DestinationType = "NMO.HTL.DST.CTY"
                                    }
                                }
                            },
                            ...
                            Rate = new WSHCEV_main.BookingServiceRate
                            {
                                ...
                                Rooms = new WSHCEV_main.BookingServiceRooms
                                {
                                    Room = v.Rooms.Select(r => new WSHCEV_main.BookingServiceRoom
                                    {
                                        ...
                                        {
                                            RoomsCount = (uint)v.VacancyReserved,
                    ...
                    rqcancelation.Details[0] = bookingService;

                    xw.Formatting = System.Xml.Formatting.None;
                    x.Serialize(xw, rqcancelation);

                    wsRnd.Timeout = 30000;
                    var response = wsRnd.HotelDescriptiveInfo(sw.ToString());
                    var sr = new StringReader(response);

                    x = new System.Xml.Serialization.XmlSerializer(typeof(WSHCEV_main.BookingServiceChargeConditionsRS));
                    var chargeListRS = (WSHCEV_main.BookingServiceChargeConditionsRS)x.Deserialize(sr);
                    sr.Close();

                    if (chargeListRS != null && chargeListRS.Item is WSHCEV_main.BookingServiceChargeConditionsRSDetails &&
                        ((WSHCEV_main.BookingServiceChargeConditionsRSDetails)chargeListRS.Item).BookingService != null)
                    {
                        foreach (var booking in ((WSHCEV_main.BookingServiceChargeConditionsRSDetails)chargeListRS.Item).BookingService)
                        {
                            //booking.BookingServiceItems.BookingServiceItem.First().ApplicationSettings
                        }
                    }
                }
                catch (Exception ex)
                ...
                try
                {
                    foreach (var vacancy in reservationModel.Vacancies)
                    {
                        ...
                        bookingService.Client = new WSHCEV_main.Client
                        {
                            ...
                            BirthDate = DateTime.Parse(reservationModel.ReservationOwner.TravelerBornDate)
                        };
                            ...
                            ((WSHCEV_main.HotelBookServiceRSDetails)lodgingListRS.Item).BookingService != null)
                        {
                            ...
                            {
                                //book.BookingServiceItems.BookingServiceItem.Select(i => i.BookingReferences.BookingReference.First().
                            }

                            if (false) // Comprobar el estado de la reserva a través de la consulta
            ...
            private List<WSHCEV_main.BookingServiceGuest> GetRequestBookingGuests(List<TravelerModel> travelers)
            {
                ...
                foreach (var traveler in travelers)
                {
                    result.Add(new WSHCEV_main.BookingServiceGuest
                    {
                        ...
                        BirthDate = DateTime.Parse(traveler.TravelerBornDate)
                    });
                }

...
namespace ArgentinahtlMVC.Models.Services
{
    public static class HtmlEnumDropDownListExtensions
    {
        ...
        public static MvcHtmlString DropDownListFor<TModel, TKey, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, Dictionary<TKey, TProperty> values)
        {
            var metadata = ModelMetadata.FromLambdaExpression(expression, htmlHelper.ViewData);
            var propType = GetNonNullableModelType(metadata);
            var converter = TypeDescriptor.GetConverter(propType);

            var items = (values != null ? values.Where(v => v.Value != null).Select(value => new SelectListItem()
            {
                Text = converter.ConvertToString(value.Value.ToString()),
